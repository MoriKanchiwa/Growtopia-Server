				...
				if (p != worlds.end()) {
							...
							for (ENetPeer* currentPeer_event = server->peers; currentPeer_event < &server->peers[server->peerCount]; ++currentPeer_event) {
								...
								for (int i_ = 0; i_ < world->drop_new.size(); i_++) {
									if (find(world->world_event_items.begin(), world->world_event_items.end(), world->drop_new[i_][0]) != world->world_event_items.end()) {
										unsigned char* raw1_ = PackBlockUpdate(14, 0, 0, 0, 0, 0, 0, 0, world->drop_new[i_][2], 0, 0, 0, 0, 0);
										for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
					...
					for (int i_ = 0; i_ < (world->npc.size() > 25 ? 25 : world->npc.size()); i_++) {
						...
						if (world->blocks[npc->x + (npc->y * 100)].fg == 4344 or world->blocks[npc->x + (npc->y * 100)].fg == 8020) {
							...
							unsigned char* raw1_ = PackBlockUpdate(34, 0, npc->x * 32 + 16, npc->y * 32 + (block == 8020 ? 6 : 16), npc->x * 32 + 16, npc->y * 32 + (block == 8020 ? 6 : 16), (int)npc->kryptis, 0, 0, 0, npc->projectile_speed, (block == 8020 ? 15 : 8), npc->uid, 2);
							for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
								if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL or pInfo(currentPeer)->world != world->name) continue;
								send_raw(currentPeer, 4, raw1_, 56, ENET_PACKET_FLAG_RELIABLE);
							}
						...
						if (itemas->pr > 0) {
							...
							if (itemas->fg == 5958) {
								...
								if (p != world->blocks.end()) {
									...
									unsigned char* raw1_ = PackBlockUpdate(17, 0x8, x_ * 32 + 16, y_ * 32 + 16, 2, 1, 2, 0, 0, 0, 0, 0, 0, 0);
									unsigned char* raw2_ = PackBlockUpdate(36, 0x8, x_ * 32 + 16, y_ * 32 + 16, 0, 0, 0, 110, 0, 0, 0, 0, 0, 0);
									for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
										if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL or pInfo(currentPeer)->world != world->name) continue;
										send_raw(currentPeer, 4, raw1_, 56, ENET_PACKET_FLAG_RELIABLE);
									...
									if (itemas->pr <= 0) {
										...
										unsigned char* raw = packPlayerMoving(&data_, 112 + alloc_(world, itemas));
										unsigned char* blc = raw + 56;
									...
									if (block_->hp == 0) {
										...
										unsigned char* raw = packPlayerMoving(&data_, 112 + alloc_(world, block_));
										unsigned char* blc = raw + 56;
										...
										unsigned char* raw1_ = PackBlockUpdate(0x8, 0x0, x_, y_, 0, 0, 0, -1, 6, x_, y_, 0, 0, 0);
										for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
											if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL or pInfo(currentPeer)->world != world->name) continue;
											send_raw(currentPeer, 4, raw1_, 56, ENET_PACKET_FLAG_RELIABLE);
										}
								...
								if (p != world->blocks.end()) {
									...
									if (items[world->blocks[a_].fg].blockType == BlockTypes::SEED and laikas == items[world->blocks[a_].fg].growTime) {
										...
										if (harvest_seed(world, block_, x_, y_, drop_count, -1)) {
											...
											dropas_(world, drop_block_);
											unsigned char* raw1_ = PackBlockUpdate(0x11, 0, drop_block_.x, drop_block_.y, 0, 108, 0, 0, 0, 0, 0, 0, 0, 0);
											for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
												if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL or pInfo(currentPeer)->world != world->name) continue;
												send_raw(currentPeer, 4, raw1_, 56, ENET_PACKET_FLAG_RELIABLE);
											}
										...
										unsigned char* raw1_ = PackBlockUpdate(17, 0x8, x_ * 32 + 16, y_ * 32 + 16, 2, 1, 2, 0, 0, 0, 0, 0, 0, 0);
										unsigned char* raw2_ = PackBlockUpdate(36, 0x8, x_ * 32 + 16, y_ * 32 + 16, 0, 0, 0, 109, 0, 0, 0, 0, 0, 0);
										for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
											if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL or pInfo(currentPeer)->world != world->name) continue;
											send_raw(currentPeer, 4, raw1_, 56, ENET_PACKET_FLAG_RELIABLE);
											send_raw(currentPeer, 4, raw2_, 56, ENET_PACKET_FLAG_RELIABLE);
					...
					if (not has_playmod2(pInfo(currentPeer), 10)) {
						...
						if (diff < 30) {
							...
							if (pInfo(currentPeer)->geiger_time + t_ < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count()) {
								...
								unsigned char* raw = packPlayerMoving(&data_);
...
int main(int argc, char* argv[]) {
	...
	if (init_enet(server_port) == -1) cout << "enet klaida" << endl;
		...
		if (server_port == 17091) domain = "mom", main_port = 17091;
	...
	ENetEvent event;
	while (true) {
		while (enet_host_service(server, &event, 1000) > 0 && (f_saving_ == false and auto_save == false)) {
			...
			case ENET_EVENT_TYPE_RECEIVE:
			{
					...
					if (pInfo(peer)->trading_with == -1) {
						if (cch == "action|dialog_return\ndialog_name|\nbuttonClicked|newbie\n\n") {
							...
							if (button_name.find("warp_to_") != string::npos) {
								...
								if (pInfo(peer)->mod == 1 || pInfo(peer)->dev == 1) {
									...
									if (button_name == "ridban") {
										...
										if (button_name.find("ban_") != string::npos && online_ == false && pInfo(peer)->dev) {
											string path_ = "db/players/" + pInfo(peer)->last_wrenched + "_.json";
								...
								if (online_ == false) {
									try {
										...
										ifstream ifs("db/players/" + name + "_.json");
										if (ifs.is_open()) {
											json j;
											ifs >> j;
							...
							if (pInfo(peer)->lastchoosenitem == 830 || pInfo(peer)->lastchoosenitem == 9164 || pInfo(peer)->lastchoosenitem == 8738 || pInfo(peer)->lastchoosenitem == 10380 || pInfo(peer)->lastchoosenitem == 9602 || pInfo(peer)->lastchoosenitem == 942 || pInfo(peer)->lastchoosenitem == 1060 || pInfo(peer)->lastchoosenitem == 1136 || pInfo(peer)->lastchoosenitem == 1402 || pInfo(peer)->lastchoosenitem == 9582 || pInfo(peer)->lastchoosenitem == 1532 || pInfo(peer)->lastchoosenitem == 3562 || pInfo(peer)->lastchoosenitem == 4774 || pInfo(peer)->lastchoosenitem == 7380 || pInfo(peer)->lastchoosenitem == 7588 || pInfo(peer)->lastchoosenitem == 8556) {
								...
								if (find_if(worlds.begin(), worlds.end(), [world](const World& a) { return a.name == world; }) != worlds.end() || not check_blast(world) || access(("db/worlds/" + world + "_.json").c_str(), 0) == 0) {
									gamepacket_t p;
										...
										if (cch.find("buttonClicked|manual") != std::string::npos) {
											...
											if (cch.find("volume|") != string::npos and cch.find("text|") != string::npos) {
												...
												if (Update) {
													...
													PlayerMoving data_{};
													data_.packetType = 5, data_.punchX = x_, data_.punchY = y_, data_.characterState = 0x8;
													unsigned char* raw = packPlayerMoving(&data_, 112 + alloc_(world_, block_));
													unsigned char* blc = raw + 56;
						...
						if (pa != top_basher_winners.end()) {
							...
							int give_premium = (place == 1 ? 75 : (place == 2 ? 50 : (place == 3 ? 15 : 15))), give_token = (place == 1 ? 25 : (place == 2 ? 15 : (place == 3 ? 10 : 0))), give_medal = (place == 1 ? 6094 : (place == 2 ? 6132 : (place == 3 ? 6130 : 0))), give_count = 1;
						...
						if (pInfo(peer)->participated == event_item && pInfo(peer)->guild_id != 0) {
							...
							if (pa != top_guild_winners.end()) {
								...
								int place = top_guild_winners[pa - top_guild_winners.begin()].first, give_premium = (place == 1 ? 75 : (place == 2 ? 50 : (place == 3 ? 15 : 15))), give_token = (place == 1 ? 25 : (place == 2 ? 15 : (place == 3 ? 10 : 0))), give_medal = (place == 1 ? 5948 : (place == 2 ? 5950 : (place == 3 ? 5952 : 0))), give_count = 1;
								pInfo(peer)->gtwl += give_premium;
								gamepacket_t p, p2;
						...
						else if (cch == "buttonClicked|weatherstore\n") send_store(peer, "weather");
						else if (cch == "buttonClicked|farmablestore\r\n") send_store(peer, "farmable");
						else if (cch == "buttonClicked|lockstore\n") send_store(peer, "locks");
						else if (cch == "buttonClicked|legendstore\n") send_store(peer, "legend");
						else if (cch == "buttonClicked|iotmstore\n") send_store(peer, "iotm");
						else if (cch == "buttonClicked|cunsumablestore\n") send_store(peer, "consumables");
						else if (cch == "buttonClicked|rareitemstore\n") send_store(peer, "rare");
						else if (cch == "buttonClicked|asseststore\n") send_store(peer, "assets");
						else if (cch == "buttonClicked|titlestore\n") send_store(peer, "title");
						else if (cch == "buttonClicked|tokenstore\n") send_store(peer, "token");
						else if (cch == "buttonClicked|summertstore\n") send_store(peer, "stoken");
						else if (cch == "buttonClicked|backstore\n") send_store(peer, "main");
						...
						if (pInfo(peer)->bypass) {
							...
							if (Server_Security.update_item_data < 15 && pInfo(peer)->world.empty()) {
								...
								enet_peer_send(peer, 0, enet_packet_create((pInfo(peer)->load_item2 ? item_data_ios : item_data), (pInfo(peer)->load_item2 ? static_cast<std::size_t>(item_data_size_ios) : static_cast<std::size_t>(item_data_size)) + 60, ENET_PACKET_FLAG_RELIABLE));
									...
									for (int i_ = 0; i_ < world_->drop_new.size(); i_++) {
										if (items[world_->drop_new[i_][0]].untradeable == 0 && world_->drop_new[i_][0] != 0 && world_->drop_new[i_][3] > 0 && world_->drop_new[i_][4] > 0 && world_->drop_new[i_][2] == uid) {
											...
											if (modify_inventory(peer, world_->drop_new[i_][0], c_) == 0) {
												...
												unsigned char* raw = packPlayerMoving(&data_);
												unsigned char* raw2 = packPlayerMoving(&data2_);
												raw2[3] = 5;
									...
									if (p != worlds.end()) {
										...
										if (to_lower(world->owner_name) == to_lower(pInfo(peer)->tankIDName) || pInfo(peer)->superdev || find(world->admins.begin(), world->admins.end(), to_lower(pInfo(peer)->tankIDName)) != world->admins.end()) {
											...
											if (modify_inventory(peer, used_item, remove) == 0) {
												...
												if (used_item == 12600) {
													for (int i_ = 0; i_ < world->blocks.size(); i_++) { if (items[world->blocks[i_].fg].blockType == SEED) world->blocks[i_].planted -= sinf64(2.592e+6); }
												}
												else {
						...
						if (p != worlds.end()) {
							...
							if (got >= 5) {
								...
								if (block_->shelf_1 >= 200) {
									...
									unsigned char* raw = packPlayerMoving(&data_, 112 + alloc);
									unsigned char* blc = raw + 56;
									form_visual(blc, *block_, *world_, peer, false);
									for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
										if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL) continue;
						...
						break;
							...
							if (pInfo(peer)->logger) {
								...
								if (t_.size() > 3) {
									button = explode("\n", t_[3])[0].c_str();
									if (button == "Empty Logs" && to_lower(pInfo(peer)->tankIDName) == "ShuTDOWN") Server_Security.logs.clear();
								}
								if (t_.size() > 5) {
									search = explode("\n", t_[5])[0].c_str();
									if (search == "Next page") {
						...
						break;
							...
							if (p != worlds.end()) {
								...
								if (t_.size() > 4) {
									string button = explode("\n", t_[3])[0].c_str();
									if (button == "filterunsigned charxt") load_vendhub(peer, world_, block_, explode("\n", t_[4])[0].c_str());
						...
						cout << item_ << endl;
						unsigned char* ex_raw = packEffectMoving37(37, item_);
						for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
							if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL or pInfo(currentPeer)->world != pInfo(peer)->world) continue;
				...
				case 4:
				{
					...
					if (pInfo(peer)->world.empty()) break;
					unsigned char* data_ = get_struct(event.packet);
								...
								case 3270: case 3496:
								{
									...
									if (steam_connector.x >= 0 and steam_connector.y >= 0) {
										...
										case 3286: //steam door
										{
											...
											unsigned char* raw = packPlayerMoving(&data_, 112 + alloc_(world_, block_s));
											unsigned char* blc = raw + 56;
																		...
																		// drop the prize
																		{
																			...
																			if (block_s->c_ * 5 <= 115) prize = 3734;
																				...
																				dropas_(world_, drop_block_);
																				{
																					...
																					data_2.YSpeed = 108;
																					unsigned char* raw2 = packPlayerMoving(&data_2);
																					gamepacket_t p;
																					p.Insert("OnConsoleMessage");
											...
											if (world_->special_event && find(world_->world_event_items.begin(), world_->world_event_items.end(), world_->drop_new[i_][0]) != world_->world_event_items.end()) {
												...
												if (c_ < 200 and world_->drop_new[i_][1] >(200 - c_)) {
													...
													if (modify_inventory(peer, world_->drop_new[i_][0], b_, false) == 0) {
														...
														data_.x = 0, data_.y = 0;
														unsigned char* raw2 = packPlayerMoving(&data_);

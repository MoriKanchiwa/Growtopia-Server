...
string fixchar2(string name) {
	string ret;
	for (int i = 0; i < name.length(); i++) if (name.compare(i, 2, "`#") == 0 || name.compare(i, 2, "``") == 0 || name.compare(i, 2, "`0") == 0 || name.compare(i, 2, "`8") == 0 || name.compare(i, 2, "`6") == 0 || name.compare(i, 2, "`9") == 0 || name.compare(i, 2, "`4") == 0 || name.compare(i, 2, "`2") == 0) i++; else ret += name[i];
	return ret;
}
...
string getItemCategory(int actionType, string name_) {
	switch (actionType) {
	case 0:
		return "Fist";
	case 1:
...
inline vector<string> explode(const string& delimiter, const string& str) {
	vector<string> arr;
	std::size_t strleng = str.length(); // buvo const int strleng = str.length();
	std::size_t delleng = delimiter.length(); // buvo const int delleng = delimiter.length();
	if (delleng == 0)
		return arr;
	std::size_t i = 0; // buvo int i = 0;
	std::size_t k = 0; // buvo int k = 0;
	while (i < strleng) {
		int j = 0;
...
int items_dat() {
	...
	//	if (def.actionType == 98) cout << def.id << endl;
		...
		if (items[i].pwl != 0) {
			if (domain != "https://gtps4.ddns.net/") items[i].pwl /= 3;
